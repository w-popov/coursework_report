...
int append_to_report(const char* format, ...) {
    ...
    if (current_len >= REPORT_BUFFER_SIZE - 1) return 0;
    
    va_list args;
    va_start(args, format);
    int written = vsnprintf(report_text + current_len, 
                           REPORT_BUFFER_SIZE - current_len, 
                           format, args);
    va_end(args);
    return written;
}
...
void load_csv_cb(Fl_Widget*, void*) {
    ...
    if (chooser.show() == 0) {
        ...
        struct IStorage *array = svector_init((struct IStorage*)&t_array_vec, 
                                               sizeof(struct TemperatureStats), 0);
        struct IStorage *errors_array = svector_init((struct IStorage*)&err_array_vec, 
        ...
        for (size_t i = 0; i < array->size(array); ++i) {
            append_to_report("%6zu]  Year: %4u,  Month: %2u,  Day: %2u,  Hours: %2u,  "
...
int main(int argc, char **argv) {
    ...
    scroll->type(Fl_Scroll::VERTICAL_ALWAYS);  // Всегда показывать полосу прокрутки
    ...
    exit_btn->callback([](Fl_Widget*, void*)
    { 
        exit(0); 
    });
